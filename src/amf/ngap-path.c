...
#if 0
int ngap_send_to_enb(amf_enb_t *enb, ogs_pkbuf_t *pkbuf, uint16_t stream_no)
{
...
int ngap_send_to_enb_ue(enb_ue_t *enb_ue, ogs_pkbuf_t *pkbuf)
{
    ...
    return ngap_send_to_enb(enb, pkbuf, enb_ue->enb_ostream_id);
}

int ngap_delayed_send_to_enb_ue(
        enb_ue_t *enb_ue, ogs_pkbuf_t *pkbuf, ogs_time_t duration)
{
    ...
    if (duration) {
        ...
        enb = enb_ue->enb;
        ogs_assert(enb);
        return ngap_send_to_enb_ue(enb_ue, pkbuf);
    }
}
        ...
        NGAP_ProcedureCode_t procedureCode, NGAP_NAS_PDU_t *nasPdu)
{
    ...
    if (enb_ue->amf_ue) {
        if (nas_eps_security_decode(enb_ue->amf_ue,
...
void ngap_send_s1_setup_response(amf_enb_t *enb)
{
    ...
    ogs_debug("[AMF] S1-Setup response");
    ngap_buffer = ngap_build_setup_rsp();
    ogs_expect_or_return(ngap_buffer);

    ogs_expect(OGS_OK ==
            ngap_send_to_enb(enb, ngap_buffer, NGAP_NON_UE_SIGNALLING));
}
        ...
        amf_enb_t *enb, NGAP_Cause_PR group, long cause)
{
    ...
    ogs_debug("[AMF] S1-Setup failure");
...
void ngap_send_initial_context_setup_request(amf_ue_t *amf_ue)
{
    ...
    rv = nas_eps_send_to_enb(amf_ue, ngapbuf);
    ...
    uint8_t action, uint32_t delay)
{
    ...
    ogs_debug("    ENB_UE_NGAP_ID[%d] AMF_UE_NGAP_ID[%d]",
            enb_ue->enb_ue_ngap_id, enb_ue->amf_ue_ngap_id);
    ...
    if (delay) {
        ...
        ngapbuf = ngap_build_ue_context_release_command(enb_ue, group, cause);
        ogs_expect_or_return(ngapbuf);

        rv = ngap_delayed_send_to_enb_ue(enb_ue, ngapbuf, delay);
        ogs_expect(rv == OGS_OK);
    } else {
        ogs_assert(action != NGAP_UE_CTX_REL_INVALID_ACTION);
        enb_ue->ue_ctx_rel_action = action;
        ...
        rv = ngap_delayed_send_to_enb_ue(enb_ue, ngapbuf, 0);
        ogs_expect(rv == OGS_OK);
    }
}
...
void ngap_send_paging(amf_ue_t *amf_ue, NGAP_CNDomain_t cn_domain)
{
    ...
    ogs_list_for_each(&amf_self()->enb_list, enb) {
        for (i = 0; i < enb->num_of_supported_ta_list; i++) {
            ...
            if (memcmp(&enb->supported_ta_list[i], &amf_ue->tai,
                ...
                rv = ngap_send_to_enb(enb, ngapbuf, NGAP_NON_UE_SIGNALLING);
                ogs_expect(rv == OGS_OK);
            }
        }
    }
        ...
        NGAP_SONConfigurationTransfer_t *SONConfigurationTransfer)
{
    ...
    rv = ngap_send_to_enb(target_enb, ngapbuf, NGAP_NON_UE_SIGNALLING);
    ogs_expect(rv == OGS_OK);
}
...
void ngap_send_handover_command(enb_ue_t *source_ue)
{
    ...
    rv = ngap_send_to_enb_ue(source_ue, ngapbuf);
    ogs_expect(rv == OGS_OK);
}
            ...
            *source_totarget_transparentContainer)
{
    ...
    ogs_assert(amf_ue);
    source_ue = amf_ue->enb_ue;
    ogs_assert(source_ue);
    ogs_assert(source_ue->target_ue == NULL);

    target_ue = enb_ue_add(target_enb, INVALID_UE_NGAP_ID);
    ogs_assert(target_ue);

    ogs_debug("    Source : ENB_UE_NGAP_ID[%d] AMF_UE_NGAP_ID[%d]",
            source_ue->enb_ue_ngap_id, source_ue->amf_ue_ngap_id);
    ...
    rv = ngap_send_to_enb_ue(target_ue, ngapbuf);
    ogs_expect(rv == OGS_OK);
}
        ...
        NGAP_Cause_PR group, long cause)
{
    ...
    ngapbuf = ngap_build_error_indication(
            amf_ue_ngap_id, enb_ue_ngap_id, group, cause);
    ogs_expect_or_return(ngapbuf);
        ...
        NGAP_UE_associatedLogicalS1_ConnectionListRes_t *partOfS1_Interface)
{
    ...
    ngapbuf = ngap_build_s1_reset_ack(partOfS1_Interface);
    ogs_expect_or_return(ngapbuf);
